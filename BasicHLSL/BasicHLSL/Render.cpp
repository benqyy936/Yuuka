...
HRESULT InitD3D11Device()
{
	...
	hr = D3DCompileFromFile(L"VS.hlsl", NULL, NULL, "VS", "vs_5_0", dwShaderFlags, 0, &pVsShaderBlob, NULL);
    ...
    InputVertex pInputVertex[] = { 
        { XMFLOAT3(-1.0, -1.0,0),XMFLOAT4(0.2f,0.6f,0.0,0.0) },
        { XMFLOAT3( -1.0, 1.0,0),XMFLOAT4(0.5f,0.2f,0.1f,0.0f) },
        { XMFLOAT3( 1.0,1.0,0),XMFLOAT4(1.0,1.0,1.0,0.0) },
        { XMFLOAT3(1.0,-1.0,0),XMFLOAT4(1.0,1.0,1.0,0.0) }
    };
    ...
    //Create Pixel Shader
    D3DCompileFromFile(L"PS.hlsl", NULL, NULL, "PS", "ps_5_0", dwShaderFlags, 0, &pPsShaderBlob, NULL);
...
HRESULT Render(HWND hwnd)
{
    ...
    viewPort.TopLeftX = (FLOAT)descSwapChain.BufferDesc.Width * 2/ 5;
    viewPort.TopLeftY = (FLOAT)descSwapChain.BufferDesc.Height *2/ 5;
    viewPort.Width = (FLOAT)descSwapChain.BufferDesc.Width/5;
    viewPort.Height = (FLOAT)descSwapChain.BufferDesc.Height/5;
    viewPort.MinDepth = 0.0;
    viewPort.MaxDepth = 1.0;
    g_pD3D11DeviceContext->RSSetViewports(1, &viewPort);
    D3D11_RASTERIZER_DESC descRast;
    ID3D11RasterizerState *pRasterize = NULL;
    memset(&descRast, 0, sizeof(descRast));
    descRast.CullMode = D3D11_CULL_BACK;
    descRast.FillMode = D3D11_FILL_SOLID;
    g_pD3D11Device->CreateRasterizerState(&descRast, &pRasterize);
    g_pD3D11DeviceContext->RSSetState(pRasterize);
